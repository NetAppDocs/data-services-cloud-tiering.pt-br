...
[role="quick-margin-para"]
<<Configurar permissões do S3,Veja como configurar permissões para o agente e para seu cluster local.>>
...
* <<Configurar permissões do S3,Certifique-se de que seu cluster ONTAP tenha permissões para acessar o bucket S3.>>
...
. Modifique a configuração do grupo de segurança associado ao agente.  Você deve alterar a política para "Personalizada" (de "Acesso Total") e deve<<Configurar permissões do S3,adicione as permissões necessárias do agente S3>> como mostrado anteriormente.
+
